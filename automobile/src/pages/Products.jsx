...
export default function AutoStore() {
    ...
    /* =====================================================
       REAL-TIME AUTO RE-FETCH (WITHOUT BROWSER RELOAD)
       - Automatically syncs when Seller creates/edits/deletes
       - Re-fetches on window focus or tab visibility change
       - Polls every 15s in background while tab is open
    ===================================================== */

    const latestFiltersRef = useRef({ searchQuery, category, page });
    useEffect(() => {
        latestFiltersRef.current = { searchQuery, category, page };
    ...
    const refreshSilently = useCallback(() => {
        ...
        fetchProducts({
            pageNumber: 1,
            append: false,
            search: latestFiltersRef.current.searchQuery,
            selectedCategory: latestFiltersRef.current.category,
            silent: true,
        });
    }, [fetchProducts]);

    useEffect(() => {
        // 1. In-app & Cross-tab product mutations from Seller page
        const unsubscribe = subscribeToProductsChanged((detail) => {
            console.log("Products changed, auto re-fetching:", detail);
            refreshSilently();
        });

        // 2. Tab focus & visibility change (throttled to 4s)
        ...
        // 3. Periodic polling interval (every 15 seconds while tab is active)
                ...
                >
                    ...
                    <div className="mb-8 flex items-end justify-between gap-6">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <span className="hidden pb-0.5 text-xs font-bold text-slate-400 lg:block">
                                {totalProducts}{" "}
                                {totalProducts === 1
                                    ? "part"
                                    : "parts"}{" "}
